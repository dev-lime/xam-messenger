...
Клиент автоматически ищет сервер в локальной сети используя несколько методов:

### Приоритет методов обнаружения

1. **mDNS (Bonjour)** — автоматическое обнаружение через сервис `_xam-messenger._tcp.local`
2. **Кэш** — ранее подключённые серверы (хранятся 24 часа)
3. **IP-сканирование** — перебор адресов в локальных подсетях
4. **Ручной ввод** — указание адреса сервера вручную

### mDNS (автоматическое обнаружение)

**Сервер** автоматически регистрируется в локальной сети через mDNS/Bonjour:
- Имя сервиса: `_xam-messenger._tcp.local`
- Порт: 8080
- TXT-записи: `version=1.0.0`, `protocol=ws`

**Клиент** (Tauri приложение) находит сервер через системный mDNS:
- macOS: встроенный Bonjour
- Windows: mdns-sd (кроссплатформенная реализация)
- Linux: mdns-sd (кроссплатформенная реализация)
...
### IP-сканирование (резервный метод)

Если mDNS недоступен (например, в веб-версии), клиент сканирует локальную сеть:

**Сканируемые диапазоны** для каждой подсети:
...
При запуске клиент автоматически подключается к первому найденному серверу.

**Кнопка "🔄 Сменить сервер"** (в шапке) открывает диалог выбора:
- Список найденных серверов с индикаторами статуса (🟢 онлайн / 🔴 офлайн)
- Источник обнаружения (📢 mDNS / 📦 кэш / 🔍 сканирование)
- Кнопка "Подключиться" для каждого сервера
- Поле для ручного ввода адреса
- Кнопка "🔄 Обновить" для повторного поиска
...
4. **Попробуйте ручной ввод адреса:**
   ...
   - Введите IP адрес сервера (например, `192.168.1.100:8080`)
