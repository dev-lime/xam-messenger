...
import { loadUserSettings, saveUserSettings, loadSession, saveSession, clearSession } from './storage.js';
import { loadLanguage, getLanguage } from './i18n.js';
import { requestPermission as requestNotifPermission } from './notifications.js';
import { setServerClient as setSc, getServerClient } from './state.js';
...
import { CONFIG, DELIVERY_STATUS } from './utils/helpers.js';
...
async function init() {
	...
	if (session) {
		...
		if (elements.serverStatus) elements.serverStatus.innerHTML = '<span style="color:var(--warning);">🔄 Восстановление сессии...</span>';
