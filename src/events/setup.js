...
import { openServerSelector, connectToServer, discoverServers, refreshServerList } from '../dialogs/server.js';
...
import { handleFileSelect, renderAttachedFiles, updateSendButton, clearMessageInput, openFile, downloadFile } from '../utils/files.js';
...
export function setupEventListeners() {
	...
	if (elements.connectionStatus) {
		elements.connectionStatus.addEventListener('click', async () => {
			...
			const session = JSON.parse(localStorage.getItem(CONFIG.STORAGE_KEYS.SESSION_USER) && localStorage.getItem(CONFIG.STORAGE_KEYS.SESSION_SERVER) ?
				JSON.stringify({user: JSON.parse(localStorage.getItem(CONFIG.STORAGE_KEYS.SESSION_USER) || '{}'), server: JSON.parse(localStorage.getItem(CONFIG.STORAGE_KEYS.SESSION_SERVER) || '{}')}) : 'null');
...
async function showLatency() {
	...
	try {
		...
		clearTimeout(timer);
		latencyEl.textContent = `⏱ ${Math.round(performance.now() - start)} мс`;
...
async function connectToManualServer() {
	...
	try {
		...
		elements.selectedServerInfo.textContent = `📡 ${ip}:${port}`;
		...
		if (e.dataTransfer?.files?.length > 0) {
			Array.from(e.dataTransfer.files).forEach(file => {
				if (file.size <= CONFIG.MAX_FILE_SIZE) {
					const attached = []; // FIXME: need access to attachedFiles
					// For now just trigger file input handler
				}
			});
		}
	}, false);
