...
### Архитектура Tauri WebSocket

В Tauri приложении используется **нативный WebSocket** через Rust (`tokio-tungstenite`), который работает без ограничений браузера:

```
┌─────────────────────────────────┐
│  WebView (UI только)            │
│  ┌───────────────────────────┐  │
│  │  invokeTauri('ws_connect')│  │
│  │  invokeTauri('ws_send')   │  │
│  │  Tauri.listen('ws_msg')   │  │
│  └───────────────────────────┘  │
└──────────────┬──────────────────┘
...
| Команда | Описание |
|---------|----------|
| `ws_connect(url)` | Подключение к серверу |
| `ws_send(message)` | Отправка сообщения |
| `ws_close()` | Закрытие соединения |

### События Tauri

| Событие | Описание |
|---------|----------|
| `ws_message` | Данные от сервера |
| `ws_connected` | Успешное подключение |
| `ws_disconnected` | Разрыв соединения |
| `ws_error` | Ошибка |

### Проверка сети

Перед подключением проверяется наличие активного сетевого интерфейса:
- Если URL локальный (`127.0.0.1`, `localhost`) — подключается напрямую
- Если URL удалённый — проверяется наличие Wi-Fi/Ethernet
- Если сети нет — выводится понятное сообщение об ошибке
...
| **Rate Limiting** | `actix-governor`, настраивается через `RATE_LIMIT` |
